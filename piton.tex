...
\PitonOptions
  { 
    ...
    detected-commands = { \highLight , \footnote , \emph , \textsl , \textbackslash } ,
...
\medskip
Note that the character \emph{quote} (U+0027 : \verb|'|) is never transformed by
\pkg{piton} in an apostrophe U+2019. There is no point loading the package
\pkg{upquote}.
...
\subsection{The double syntax of the command \texttt{\textbackslash piton}}
...
\subsection{The keys of the command \texttt{\textbackslash PitonOptions}}
...
The key |font-command| has also an ``additive syntax''. That means that it's
possible to write |font-command +=| $\langle$\textsl{instructions}$\rangle$
and the tokens $\langle$\textsl{instructions}$\rangle$ will be added
on the right of the current value of the parameter.

In that case, the new formating instructions are added at the right of the current
...
\item The key \Definition{line-numbers/step} should be used when we don't want
to print all the numbers of line. If $n$ is the value of that key, only one
number out of~$n$ will be printed. Of course, the initial value is 1.
...
The key |font-command| has also an ``additive syntax''. That means that it's
possible to write |format +=| $\langle$\textsl{instructions}$\rangle$
and the tokens $\langle$\textsl{instructions}$\rangle$ will be added
on the right of the current value of the parameter.

\item \colorbox{yellow!50}{\textbf{New 4.11}}

\nobreak
The key \Definition{line-numbers/position} specifies the position of the numbers.
Two values are possible: |left| and |right|. The initial value is |left|.


\item \colorbox{yellow!50}{\textbf{New 4.13}}

\nobreak
With the key \Definition{line-numbers/lmmono10-drawn}, the numbers of lines use the
font \emph{Latin Modern Mono 10}, but the numbers are drawn and are not displayed as text.
Therefore, during a copy-paste, the \textsc{pdf} readers won't catch those numbers of lines.
...
\colorbox{yellow!50}{\textbf{Nouveau 4.11}}

\nobreak
The key |right-margin| is similar to the previous one, but for the margin on the
right.
...
\item the width of the colored backgrounds added by |\rowcolor|
...
\fvset{commandchars=\~\&\@}
\begin{Verbatim}
~emphase&\begin{Piton}[language=C,line-numbers,line-numbers/lmmono10-drawn,gobble@
~emphase&              ,background-color=gray!15,rounded-corners,width=min,splittable=4]@
...
\begingroup
\PitonOptions{language=C,line-numbers,line-numbers/lmmono10-drawn,
   gobble=4,background-color = gray!15,rounded-corners,width=min}
    \begin{Piton}
    void bubbleSort(int arr[], int n) {
        int temp;
        int swapped;
...
\textsl{key=value} pairs. The keys are names of styles and the values are LaTeX
...
The extension \pkg{piton} provides a special style called~\Definition{UserFunction}.
...
It's possible to use |\NewEnvironmentCopy| on the environment |{Piton}| but
...
|d|, |s| and |m|), |morecomment| (with the letters |i|, |l|, |s|, |n| and |f|\footnote{For the letter |f|, only a specification for a comment beginning in the first column is allowed.}),
...
\begin{Verbatim}
\NewPitonLanguage{LaTeX}{keywordsprefix = \ , alsoother = ~@_ , morecomment = [l]\%}
\end{Verbatim}

Initially, the characters |@| and |_| are considered as letters (in the
sens of \pkg{listings} and not in the sens of the TeX's catcodes!) because,
in many computer languages, they are allowed in the keywords and the names
of the identifiers. With \verb|alsoother = @_|, we retrieve them from the
...
the \textsc{pdf} generated by LuaLaTeX.
...
listings produced by |\PitonInputFile|. {\bfseries The initial value of that parameter is
|true| (and not |false|).}
...
the listing or within the $n$ last lines.\footnote{Remark that we speak of thes
...
should imagine that he has to compose a computer listing which contains
...
empty line in the final \textsc{pdf} (this vertical space is deleted when it
...
\Definition{split-separation~+} to add elements on the right of the parameter
|split-separation|. The name of that key contains a space in order to be able
to write, for example, |split-separation += \hrule|.
\end{itemize}
...
\begin{Verbatim}
\begin{Piton}[
split-on-empty-lines, ~emphase#split-separation += \PitonOptions{resume}@ , background-color=gray!15, line-numbers ]
def square(x):
    """Computes the square of x"""
    return x*x
...
\item The names of the LaTeX commands may appear \emph{with} or \emph{without}
the leading backslash (eg. |detected-commands = { \emph, textbf }|).
...
\begin{Verbatim}
\PitonOptions{~emphase#vertical-detected-commands@ = \newpage}
\end{Verbatim}
...
with one mandatory argument, written between explicit braces).
...
\bigskip
\index{before-end-escape}
\index{after-begin-escape}
\colorbox{yellow!50}{\textbf{New 4.14}}

\smallskip
\nobreak
It's possible to use the keys \Definition{after-begin-escape} and
\Definition{before-end-escape} in order to insert some LaTeX material at the
beginning of at the end of the LaTeX escape.
...
return in the mandatory arguments of these functions.
...
\medskip
For examples of use, see the example of formatting of LaTeX code,
part~\ref{latex-formatting}, p.~\pageref{latex-formatting}, and the part
concerning |pyluatex|, part~\ref{pyluatex}, p.~\pageref{pyluatex}.
...
\bigskip
It's possible to have the numbers of lines on the right with
|line-numbers/position=right| (the initial value of the parameter
|line-numbers/position| is, without surprise, |left|). In that case, it is good
to provide a positive value to |right-margin| (it's possible to use
|right-margin=auto| but the actual value computed for |right-margin| will be
probably appear to be too small if there is long broken lines of code.
...
\subsection{The command \texttt{\textbackslash rowcolor}}
...
\bigbreak
Here is now the same example with the use of the key |background-color|
...
column on the left for the numbers of lines.
...
Remark that we must use |\newpage{}| and not |\newpage| because the LaTeX
commands detected by \pkg{piton} are meant to be commands with one argument
(between explicit curly braces).
...
\subsection{Formatting of LaTeX code}

\label{latex-formatting}

Here is a way to define an environment |{CodeLaTeX}| which formats a LaTeX code and also
produces the output obtained when compiling that code with LaTeX

\medskip
\begin{Code}
\NewPitonLanguage{LaTeX}{keywordsprefix = \ , alsoother = @_ , morecomment = [l]\%}

\NewPitonEnvironment{\emph{CodeLaTeX}}{}
  { \PitonOptions{language=LaTeX} }
  { \luadirect{tex.print(piton.get_last_code():explode(string.char(10)))} }

\begin{\emph{CodeLaTeX}}
    \[x = \frac{-b \pm \sqrt{b^2-4ac}}{2a}\] % solution
\end{\emph{CodeLaTeX}}
\end{Code}
...
\textcolor{red}{Caution: The following examples require the version 0.7.0 of
  \pkg{pyluatex}.}
...
\bigskip
It's also possible to use, in that environment, the mechanims for escape to LaTeX
...
\bigskip
\subsubsection{Use of the environment \{python\} of pyluatex with the key repl}

\label{python}
\index{python@\texttt{\{python\}} (environment of \pkg{pyluatex})}

With the key |repl| the environment |{python}| of \pkg{pyluatex} submit its
content to Python and return what we obtain when we submit that code to a
\textsc{repl} (\emph{read-eval-print loop}) of Python. We obtain a succession of
instructions preceded by the prompt |>>>| of Python and values returned by
Python (and the outputs of potential commands |print| of Python).
...
of course, the output of |{python}| must be treated by \pkg{piton}. Therefore, it's not
possible to use the escaping tools (|detected-commands|, |begin-escape|, etc.) in the code.

\begin{Verbatim}
\NewDocumentEnvironment { PitonREPL } { ! O { } } % the ! is mandatory
 { 
   \PitonOptions
     {
       ~emphase#prompt-background-color=blue!15@ ,
       ~emphase#background-color=none@, % for small margins
       ~#1
     }
   \PyLTVerbatimEnv
   \begin{python}[repl,quiet]
 }
 {
   \end{python}
   \directlua
...
Delim    & the delimiters |(|, |[|, \texttt{\{}, |)|, |]| and \texttt{\}} \\      
...
\begin{tabularx}{0.9\textwidth}{@{}>{\ttfamily}l>{\raggedright\arraybackslash}X@{}}
  ...
  Keyword.Governing & the following keywords: |and|, |begin|, |class|, |constraint|,
                     ...
                     |struct|, |type| and |val|. \\
  Identifier & the identifiers \\
  Delim    & the delimiters |(|, |[|, \verb+[|+, |)|, |]| and \verb+|]+ \\
  Punct    & the symbols of ponctuation |,| |:|, |;| and |!|. \\
  \bottomrule
\end{tabularx}
\end{center}
...
|extern|, |for|, |goto|, |if|, |noexcept|, |private|, |public|, |register|,
|restrict|, |try|, |return|, |static|, |static_assert|, |struct|, |switch|,
...
\newpage
\subsection{The languages defined by \texttt{\textbackslash NewPitonLanguage}}
...
\medskip
The successive versions of the file |piton.sty| provided by TeX Live are also available
on the \textsc{svn} server of TeX Live:
...
\subsection*{Changes between versions 4.13 and 4.14}

The key |line-numbers/format| has now an additive syntax.

New keys |after-begin-escape| and |before-begin-escape|.

\subsection*{Changes between versions 4.12 and 4.13}

New key |line-numbers/lmmono10-drawn|.

\subsection*{Changes between versions 4.11 and 4.12}

The documentation has been changed in order to be compatible with the version
0.7.0 of \pkg{pyluatex}.

New key ``|font-command +|''.

\subsection*{Changes between versions 4.10 and 4.11}

New keys |right-margin| and |line-numbers/position|.


\subsection*{Changes between versions 4.9 and 4.10}
