...
contenu de l'environnement courant dans ce fichier. À la première utilisation du fichier par \pkg{piton}, celui-ci
est effacé.
...
\NewDocumentEnvironment { ~emphase#PitonExecute@ } { ! O { } }  % le ! est obligatoire 
  ...
  }
  {
    ...
    \lua_now:n
      {
        tex.print("\\begin{Piton}[~#1]")
        tex.print(pyluatex.get_last_code())
        tex.print("\\end{Piton}")
        tex.print("") 
      }
    \begin{center}
      \lua_now:n { tex.print(pyluatex.get_last_output()) }
    \end{center}
  }
\ExplSyntaxOff
\end{Verbatim}

% On utilise \lua_now:n et non pas \luadirect car on ne peut pas d'expanson de l'argument
% En effet, si on fait une expansion de l'argument, la commande \\ risque d'être développée
% Habituellement, en LaTeX, la commande \\ est protégée mais pas si on charge lineno (chargé
% par exemple par minted).
...
\indexcommand{g\_piton\_last\_code\_tl}
\index{piton.last@\texttt{piton.last\_code} (variable Lua)}
...
\smallskip
L'extension \pkg{piton} fournit deux moyens pour accéder au contenu du dernier environnement de \pkg{piton} : la
variable Lua |piton.last_code| et la variable L3 |\g_piton_last_code_tl|. En fait, dans les deux cas, il s'agit du
corps de l'environnement \emph{après application d'une éventuelle clé} |gobble| (cf. p.~\pageref{gobble}).

\begin{itemize}
\item La variable Lua \Definition{piton.last\_code} est une chaîne de caractères de Lua. Les retours à la ligne
(présents dans l'environnement de départ) apparaissent comme des caractères |\r| (c'est-à-dire des caractères
U+000D).


\item La variable L3 \Definition{\textbackslash g\_piton\_last\_code\_tl} est la version L3 de la variable Lua
précédente.  
\begin{itemize}
\item Comme l'indique le préfixe |g| dans le nom, elle doit être utlisée comme une variable \emph{globale}
(au sens de TeX). 

\item Comme son nom est |\g_piton_last_code_tl| et non |\g__piton_last_code_tl|, il s'agit d'une variable
\emph{publique} (et c'est pourquoi elle est documentée ici).

\item Comme l'indique le suffixe |tl|, il s'agit d'une \emph{token list} de L3. Néanmoins, tous les \emph{tokens}
de cette liste sont des caractères de catcode «other» (=12) y compris les espaces et les retours à la ligne.
\end{itemize}
\end{itemize}
