...
La valeur initiale de cette clé est |true|.
...
\colorbox{yellow!50}{\textbf{Nouveau 2.7}}\enskip L'extension \pkg{piton} fournit une option
\Definition{split-on-empty-lines}. Quand cette clé est active, les listings sont découpés en morceaux au niveau des
lignes vides\footnote{Les lignes considérées comme vides sont celles qui ne comportent que des espaces.} du code et
chaque morceau est traité par \pkg{piton} comme un listing autonome avant d'être envoyé à 
...
\smallskip
La clé \Definition{split-separation} permet de spécifier une liste de tokens LaTeX qui sera insérée entre les
morceaux. La valeur initiale est |\vspace{\baselineskip} \vspace{-1.25pt}|. Avec cette valeur, les morceaux de code
s'enchaînent avec une ligne vides entre les deux (même si, en fait, plusieurs lignes vides les séparaient dans
le code initial).
