...
\Definition{old-PitonInputFile} dans le préambule du document LaTeX pour avoir l'ancien
comportement de |\PitonInputFile| (néanmoins, cette clé sera supprimée dans une prochaine
version de \pkg{piton} !).
...
$n$~dernières.\footnote{Remarquer que l'on parle des lignes du listing d'origine, une
  telle ligne pouvant être composée sur plusieurs lignes dans le PDF final si la clé
  |break-lines-in-Piton| est utilisée.}
...
Chaque morceau du code informatique est formaté de manière autonome par un environnement
|{Piton}|, avec éventuellement sa propre numérotation des lignes (si la clé |line-numbers|
est active) et son propre fond coloré (si la clé |background-color| est utilisée) séparé
des fonds des autres morceaux. Si elle est active, la clé |splittable| s'applique de
manière autonome dans chaque morceau.
...
automatiquement détectés par \pkg{ipton} ne doivent pas enjamber les lignes vides du
listing de départ. 
