...
\subsection{La double syntaxe de la commande \texttt{\textbackslash piton}}
...
\subsection{Les clés de la commande \texttt{\textbackslash PitonOptions}}
...
Cette clé possède aussi une syntaxe additive, c'est-à-dire que l'on peut
écrire : |font-command +=| $\langle$\textsl{instructions}$\rangle$. Les
tokens $\langle$\textsl{instructions}$\rangle$ sont alors ajoutées à droite
de la valeur courante du paramètre.
...
numérotation des lignes dans les environnements |{Piton}| et dans les listings
...
\item La clé \Definition{line-numbers/step} doit être utilisée quand on ne veut
pas faire afficher tous les numéros de ligne. Si $n$ est la valeur de cette clé,
les numéros ne seront affichés que de~$n$ en~$n$. Bien sûr, la valeur initiale
vaut~$1$.
...
Cette clé possède aussi une syntaxe additive, c'est-à-dire que l'on peut
écrire : |format +=| $\langle$\textsl{instructions}$\rangle$. Les
tokens $\langle$\textsl{instructions}$\rangle$ sont alors ajoutées à droite
de la valeur courante du paramètre.

\item La clé \Definition{line-numbers/position} indique la position des numéros
: à gauche, avec la valeur |left| ou bien à droite avec la valeur |right|. La
valeur initiale est |left|.


\item \colorbox{yellow!50}{\textbf{Nouveau 4.13}}

\nobreak
Avec la clé \Definition{line-numbers/lmmono10-drawn}, les numéros de lignes
utilisent la fonte \emph{Latin Modern Mono 10} mais les chiffres sont dessinés
et non affichés comme du texte. Ainsi, lors d'un copier-coller, les lecteurs de
\textsc{pdf} ne captureront pas ces numéros de lignes.
...
\smallskip
La clé |background-color| accepte aussi en argument une \emph{liste} de couleurs
(séparées par des virgules). Les lignes sont alors coloriées de manière cyclique
avec ces couleurs.
...
la partie \ref{python} p.~\pageref{python}.
...
~emphase#              background-color=gray!15rounded-corners,width=min,splittable=4]@
...
\begin{Piton}[language=C,line-numbers,line-numbers/lmmono10-drawn,gobble,
...
qualifiées de \emph{semi-globales}). Il est aussi possible de mettre, \emph{à la fin de la liste
  d'instructions}, une commande LaTeX prenant exactement un argument (obligatoire).
...
\bigskip
Il existe un style spécial nommé \Definition{UserFunction}. Ce style s'applique aux noms des
...
|\NewPitonLanguage| sont : |morekeywords|, |otherkeywords|, |sensitive|,
|keywordsprefix|, |moretexcs|, |morestring| (avec les lettres |b|, |d|, |s| et
|m|), |morecomment| (avec les lettres |i|, |l|, |s|, |n| et |f|\footnote{Pour la lettre |f|, seule une spécification de commentaire commençant en première colonne est autorisée.})),
|moredelim| (avec les lettres |i|, |l|, |s|, |*| et |**|), |moredirectives|,
|tag|, |alsodigit|, |alsoletter| et |alsoother|.
...
Initialement, les caractères |@| et |_| sont considérés comme des lettres (au
sens de \pkg{listings} et non au sens des catcodes de TeX !) car de nombreux
langages de programmation les autorisent dans les mots-clés et les
identificateurs. Avec \verb|alsoother = @_|, on les retire de la catégorie des
lettres.
...
\bigskip
\textbf{Avec les numéros de lignes}
...
\bigskip
Avec la commande |\SetPitonIdentifier|, on peut ajouter à un langage informatique de
...
\item Les noms de commandes LaTeX peuvent apparaître \emph{avec} ou \emph{sans}
la contre-oblique (ex. : |detected-commands = { \emph , textbf }|).
...
\colorbox{yellow!50}{\textbf{Nouveau 4.14}}

\smallskip
\nobreak
On peut utiliser les clés \Definition{after-begin-escape} et
\Definition{before-end-escape} pour insérer du matériel LaTeX au début ou à la
fin de l'échappement LaTeX.
...
commandes LaTeX déclarées par la clé |detected-commands| et ses variantes (cf.
...
\medskip
Pour un exemple d'utilisation, voir l'exemple~\ref{latex-formatting},
p.~\pageref{latex-formatting}, ainsi que la partie concernant l'utilisation
...
\subsection{La commande \texttt{\textbackslash rowcolor}}
...
\subsection{Formatage de code LaTeX}

\label{latex-formatting}

Voici une manière de définir un environnement |{CodeLaTeX}| qui formate du code
LaTeX et affiche dessous le résultat obtenu en compilant ce code avec LaTeX.
...
\subsubsection{Utilisation de l'environnement \{python\} de pyluatex avec la clé repl}

\label{python}

\index{python@\texttt{\{python\}} (environnement de \pkg{pyluatex})}

Avec la clé |repl|, l'environnement |{python}| de \pkg{pyluatex} passe son
contenu à Python et renvoie ce que l'on obtient quand on fournit ce code à une
boucle \textsc{repl} (\emph{read-eval-print loop}) de Python. On obtient un
entrelacement d'instructions précédées par le prompt |>>>| de Python et des
valeurs renvoyées par Python (et de ce qui a été demandé d'être affiché avec des
\piton{print} de Python).
...
car, bien sûr, c'est le retour fait par |{python}| qui doit être traité par
...
Delim      & les délimiteurs |(|, |[|, \texttt{\{}, |)|, |]| et \texttt{\}} \\  
...
\begin{tabularx}{0.9\textwidth}{@{}>{\ttfamily}l>{\raggedright\arraybackslash}X@{}}
  ...
  Keyword.Governing & les mot-clés suivants : |and|, |begin|, |class|, |constraint|,
                      ...
                      |struct|, |type| et |val|. \\
  Identifier & les identificateurs \\
  Delim      & les délimiteurs |(|, |[|, \verb+[|+, |)|, |]| et \verb+|]+ \\
  Punct      & les symboles de ponctuation |,| |:|, |;| et |!|. \\
  \bottomrule
\end{tabularx}
\end{center}
...
\subsection{Les langages définis par la commande \texttt{\textbackslash NewPitonLanguage}}
