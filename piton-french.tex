...
\texttt{\textbackslash verb} où l'argument est délimité entre deux caractères 
...
\item \index{write} La clé \Definition{write} prend en argument un nom de
...
\item \index{join} \colorbox{yellow!50}{\textbf{Nouveau 4.4}}
...
\item \index{print} \colorbox{yellow!50}{\textbf{Nouveau 4.5}}
...
|\PitonInputFile| (ce fond a une largeur que l'on peut fixer avec la clé |width|
décrite ci-dessous). La clé |background-color| accepte une couleur définie
...
\item \index{width} La clé \Definition{width} fixe la largeur du listing
produit. Cette largeur s'applique aux fonds colorés spécifiés par les clés
|background-color| et |prompt-background-color| et également quand une coupure
automatique des lignes est demandée par |break-lines| (cf.~\ref{line-breaks},
p.~\pageref{line-breaks}).

Cette clé peut prendre comme valeur une longueur explicite mais aussi la valeur
spéciale~|min|. Avec cette valeur, la largeur sera calculée à partir de la
largeur maximale des lignes de code. Attention : dans le cas où des fonds
colorés sont utilisés, l'usage de cette valeur spéciale~|min| requiert deux
compilations LuaLaTeX\footnote{La largeur maximale est calculée lors de la
  première compilation, écrite sur le fichier~|aux|, puis réutilisée lors de la
  compilation suivante. Certains outils comme |latexmk| (utilisé par Overleaf)
  effectuent automatiquement un nombre suffisant de compilations.}.

Pour un exemple d'utilisation de |width=min|, voir la
partie~\ref{example-comments} sur les exemples, p.~\pageref{example-comments}.
...
intervenir sur ces espaces matérialisés, même si la clé
...
\bigskip
Voir d'autres exemples d'utilisation avec \pkg{tcolorbox} à la page
\pageref{examples-tcolorbox}.
...
\colorbox{yellow!50}{\textbf{Nouveau 4.3}}
...
Pour utiliser \pkg{piton} avec \pkg{tcolorbox}, on charge \pkg{tcolorbox} (bien sûr) puis on
crée de nouveaux environnements avec la commande |\NewPitonEnvironment|.

\medskip
Si on souhaite une boîte à la largeur courante (|\linewidht|), la programmation est simple.
Si on souhaite que la boîte puisse être coupée par un saut de page, il faut à la fois utiliser
la clé |breakable| de \pkg{tcolorbox} (ce qui implique d'avoir chargé la bibliothèque
\pkg{breakable} de \pkg{tcolorbox}: |\tcbuselibrary{breakable}|) et la clé |splittable| de
\pkg{piton} (cf.~p.~\pageref{breakable}).
...
Bien sûr, si on veut changer la couleur du fond, on n'utilise pas |background-color|
de \pkg{piton} mais les outils fournis par \pkg{tcolorbox} (|colback| pour la couleur
du fond).


\bigskip
Si on souhaite une boîte de largeur ajustée à la largeur naturelle du listing, il faudra
utiliser la clé |capture=hbox| (alias: |hbox|) de \pkg{tcolorbox} et l'environnement
ne sera pas sécable par un saut de page, même avec la clé |breakable| (c'est là une
caractéristique de \pkg{tcolorbox}).

On programme avec la clé |width=min| de \pkg{piton}, on encapsule l'environnement
|{Piton}| dans un environnement |{varwidth}| de l'extension \pkg{varwidth} (qu'il faut avoir
chargé), on stocke le tout dans une boîte LaTeX (avec l'environnement |{lrbox}| de LaTeX)
et on réutilise cette boîte dans un environnement |{tcolorbox}| dans la dernière partie de
|\NewPitonEnvironment|.
...
\vspace{1cm}
Voici un exemple plus sophistiqué avec une colonne colorée pour les numéros de ligne.
Cet exemple requiert que la librairie \pkg{skins} de TikZ soit chargée :
...
\begin{Verbatim}
~emphase#\NewPitonEnvironment@{Python}{m}
{%
\PitonOptions
  {
    width=min,
...
\begin{BVerbatim}[boxwidth=8cm,baseline=c]
...
On rappelle qu'une telle boîte ne peut pas être coupée par un saut de page
(même en utilisant la bibliothèque \pkg{breakable} de \pkg{tcolorbox}).
...
une traduction anglaise de la documentation ici présente, ainsi que le
code source commenté et un historique des versions.
