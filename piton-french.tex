...
largeur maximale des lignes de code.
...
~emphase&     splittable=4,gobble,background-color=gray!15,width=min]@
...
\begin{Piton}[language=C,line-numbers,gobble,background-color = gray!15,width=min]
    void bubbleSort(int arr[], int n) {
        int temp;
        int swapped;
        for (int i = 0; i < n-1; i++) {
...
\medskip
Si on souhaite une boîte à la largeur courante (|\linewidth|), la programmation est simple.
