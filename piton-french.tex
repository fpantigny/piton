...
\medskip
\begin{Code}
\emph{\SetPitonStyle{String.Doc = \rowcolor{gray!15}\color{black!80}}}
...
\medskip
\begin{Code}
\begin{Piton}\emph{[background-color=gray!30,rounded-corners,width=min,splittable=4]}
def carré(x):
    """Calcule le carré de x"""
    return x*x
def carré(x):
...
\begin{Piton}[
   ...
   \emph{add-to-split-separation = \PitonOptions{resume}} ,
